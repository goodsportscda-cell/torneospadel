...
export function PlayerUpcomingMatches({ jugadorId }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // 2. Buscar partidos de zona (pendientes/programados)
        const estadosPendientes = ["programado", "pendiente", "en_curso"];
        const { data: pz1 } = await supabase
          .from("partidos_zona")
          .select("id, pareja_local_id, pareja_visitante_id, fecha_hora, zona_id, estado")
        ...
        // 3. Buscar partidos de llave (pendientes/programados)
        const { data: pl1 } = await supabase
          .from("partidos_llave")
          .select("id, pareja_local_id, pareja_visitante_id, fecha_hora, llave_id, ronda, estado")
          .in("pareja_local_id", misInscripcionesIds)
          .in("estado", estadosPendientes);
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0 ml-2">
                  <Badge 
                    variant="outline"
                    className="text-[10px] h-5 px-2 bg-background font-bold text-primary border-primary/30"
                  >
                    Programado
                  </Badge>
                  {p.fecha ? (
                    ...
                    <span className="text-[10px] text-muted-foreground italic">A conf.</span>
                  )}
                </div>
              </div>
            );
