...
async function main() {
  const { data: llaves } = await supabase.from('llaves').select('id, torneo_id').limit(1);
  if (!llaves || llaves.length === 0) return;
  
  const llaveId = llaves[0].id;
  const { data: partidos } = await supabase.from('partidos_llave').select('id, numero').eq('llave_id', llaveId).limit(1);
