...
async function checkZonasSets() {
  const { data: sets, error } = await supabase.from('sets_partido').select('*').not('partido_id', 'is', null).order('created_at', { ascending: false }).limit(5);
  console.log("Recent zona sets:");
  console.log(sets);
}
checkZonasSets().catch(console.error);
