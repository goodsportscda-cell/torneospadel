...
async function checkCuartos() {
  const { data: p } = await supabase.from('partidos_llave').select('*').eq('llave_id', 'a6c6be45-aa79-4471-a75c-f1511de016b4').in('numero', [17, 18, 19, 20]);
  if (!p) return;
  p.forEach(m => console.log(`M${m.numero}: ${m.ref_local} vs ${m.ref_visitante}`));
}
checkCuartos().catch(console.error);
