...
const buttonNew = `                    <div className="flex items-center gap-2">
                      <Button onClick={() => handleGenerarFechaRegular(selectedFechaNum)}>
                        <CalendarDays className="h-4 w-4 mr-2" />
                        Generar Cruces por Regla Ascensos/Descensos (Fecha {selectedFechaNum})
                      </Button>
...
const buttonNew2 = `                    <div className="flex items-center gap-2">
...
if (content.includes(buttonTarget)) {
  ...
  console.log("Error: button 1 block not found");
}

if (content.includes(buttonTarget2)) {
  content = content.replace(buttonTarget2, buttonNew2);
} else {
  console.log("Error: button 2 block not found");
}
...
console.log("Success: applied edit cruces modal");
