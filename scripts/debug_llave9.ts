...
async function debug() {
  ...
  const [{ data: zs }, { data: zp }, { data: pz }, { data: sets }] = await Promise.all([
      ...
      supabase.from('zonas_parejas').select('*').eq('torneo_id', t_id),
      supabase.from('partidos_zona').select('*').in('zona_id', (await supabase.from('zonas').select('id').eq('torneo_id', t_id)).data.map(z=>z.id)),
