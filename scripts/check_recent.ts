...
async function checkRecentMatches() {
  // Buscar un partido que fue actualizado recientemente
  const { data: p } = await supabase.from('partidos_llave').select('*').order('updated_at', { ascending: false }).limit(5);
  console.log("Recent partidos:", p);
  
  if (p && p.length > 0) {
    const { data: sets } = await supabase.from('sets_partido').select('*').eq('partido_llave_id', p[0].id);
    console.log(`sets para p[0] (${p[0].id}):`, sets);
  }
}
